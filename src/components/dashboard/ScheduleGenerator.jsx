...
const ScheduleGenerator = ({ onClose }) => {
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Pilih toko + Cari karyawan */}
            {(loadingStores || stores.length >= 1) && (
            <div
                className={`mb-4 ${
                ['manual', 'hybrid'].includes(generationType) ? 'flex items-end gap-2' : ''
                }`}
            >
                {/* Pilih Toko */}
                ...
                {/* Cari Karyawan → hanya manual/hybrid */}
            ...
            {/* Banner Fokus (tetap di bawah baris tadi) */}
            {['manual', 'hybrid'].includes(generationType) && focusedEmployeeId && (
            <div className="flex items-center justify-between bg-blue-50 text-blue-800 border border-blue-200 rounded-lg px-3 py-2 mb-4">
                <div>
